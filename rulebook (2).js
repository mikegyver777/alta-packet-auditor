...
================ READING CHECKBOXES & MARKS ON SCANS (IMPORTANT) =================
On phone-scanned packets, checkmarks and initials are often light, gray, partial,
or slightly outside the box. A faint or partial mark still COUNTS as checked.
Only treat a box/line as unchecked when it is CLEARLY and completely empty. When
you are unsure whether a checkbox is marked, assume it IS marked and do NOT flag
it. Never flag a missing checkmark, initial, or signature based on a faint scan —
only when the space is definitely, visibly blank. This applies to: the Three-Day
initial, Paid/Upon Completion boxes, Terms circles, credit-check initials, and all
signatures.
